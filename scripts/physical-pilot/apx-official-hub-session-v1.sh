...
[[ -x /usr/bin/pipewire && -x /usr/bin/wireplumber && -x /usr/bin/pipewire-pulse ]]
...
pulse_pid=$!
for _ in {1..200}; do
  [[ -S $RUNTIME/pipewire-0 && -S $RUNTIME/pulse/native ]] && break
  kill -0 "$pipewire_pid"; kill -0 "$wireplumber_pid"; kill -0 "$pulse_pid"
  /usr/bin/sleep 0.05
done
[[ -S $RUNTIME/pipewire-0 && -S $RUNTIME/pulse/native ]]
