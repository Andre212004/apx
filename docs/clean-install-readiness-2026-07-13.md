...
pure contracts advanced; no host or disk effect is authorized.
...
4. No minimum-privilege mutating executor, broker, PAM owner-authentication
   service, trusted evidence store, or approval authority exists.
5. Pure ordered state machines now exist, but no real effect adapters exist for
   GPT, ESP, LUKS2, Btrfs, package installation, systemd-boot, or reboot
   verification.
6. No disposable VM/spare-disk rehearsal has tested interruption before and
   after each destructive effect.
7. C1–C6 have not passed on a fresh installation, including hostile local-root
   and two-Environment independent-package tests.
...
3. Select the exact cryptographic/tool profile, perform the documented physical
   ...
   generation.
4. Implement minimum-privilege effect adapters only after their pure models and
   failure tests pass.
5. Supply the missing VM/ISO/bootstrap/trust identities, create an explicitly
   disposable VM, generate its dossier, and execute the closed C0–C6 rehearsal.
6. Collect final target facts and generate a fresh target-bound dossier.

## Installation Decision

Starting a real Arch installation now would produce a minimal Arch system but
not a functional APX orchestration layer. It would strand the user between a
formatted disk and unimplemented Hub/executor behavior. The correct decision is
therefore `blocked`, not “install and fix it live.”

The first safe user-visible milestone is a complete disposable-machine C0–C6
rehearsal. Only its exact successful artifacts and failure evidence may support
a real-machine dossier.
